...
Wicket.AutoComplete=function(elementId, callbackUrl, cfg, indicatorId){    
    ...
    function setSelected(newSelected) {
        if (newSelected != selected) {
            ...
            localThrottler.throttle(getMenuId(), throttleDelay, sendSelectedValue);
        }
    }
    ...
    function sendSelectedValue() {
        ...
        Wicket.Log.info("Send ajax request:" + value);
        wicketAjaxGet(callbackUrl+(callbackUrl.indexOf("?")>-1 ? "&" : "?") + "sv="+processValue(value),
            null, null, null, "wicket-autocomplete|d");
    }
    ...
    function actualUpdateChoices()
    {
        ...
        var request = new Wicket.Ajax.Request(callbackUrl+(callbackUrl.indexOf("?")>-1 ? "&" : "?") + "q="+processValue(value),
